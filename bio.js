...
const bioFragSource = `
  ...
  void main() {
      ...
      // Force mask to be fully opaque if in whole page mode
      if (u_isWholePage > 0.5) {
          maskVal = 1.0;
      } else if (maskVal < 0.01) {
...
function setupBio() {
  if (!bioWebglLayer) {
    bioWebglLayer = createGraphics(width, height, WEBGL);
  ...
  if (!bioMaskLayer) {
    bioMaskLayer = createGraphics(width, height);
...
function windowResizedBio() {
  if (bioWebglLayer) bioWebglLayer.resizeCanvas(width, height);
  if (bioMaskLayer) bioMaskLayer.resizeCanvas(width, height);
  initBioSimulation(true);
}
...
function initBioSimulation(rebuildArrays = false) {
  ...
  bioMaskLayer.clear();
  bioMaskLayer.background(0); 
  
  if (hrW > 0 && hrH > 0 && !isWholePageMode) {
    bioMaskLayer.image(logoImg, hrX, hrY, hrW, hrH); 
  }

  if (rebuildArrays) {
    bioSimScale = isWholePageMode ? 4 : 3;

    bioSimWidth = max(floor(width / bioSimScale), 1);
    bioSimHeight = max(floor(height / bioSimScale), 1);
  ...
  if (isWholePageMode) {
    // FIX: Add 1-pixel safe padding around the screen to prevent array out-of-bounds NaN math crash
    for (let y = 1; y < bioSimHeight - 1; y++) {
        for (let x = 1; x < sw - 1; x++) {
            let i = x + y * sw;
            bioValidSpots.push(i);
            
            // Edges are now safely clamped slightly inward from the absolute boundary
            if (x === 1 || x === sw - 2 || y === 1 || y === bioSimHeight - 2) {
                bioEdgeMask[i] = 1;
            }
        }
    }
  ...
  let seedCount = isWholePageMode ? 60 : 25;
...
function drawBio() {
  ...
  if (bioValidSpots.length > 0) {
    let drawSeedCount = isWholePageMode ? 12 : 4;
    for (let k = 0; k < drawSeedCount; k++) {
  ...
  bioRenderShader.setUniform('u_maskTex', bioMaskLayer); 
  bioRenderShader.setUniform('u_resolution', [width, height]);
  bioRenderShader.setUniform('u_time', millis() / 1000.0);
  bioRenderShader.setUniform('u_isWholePage', isWholePageMode ? 1.0 : 0.0);
